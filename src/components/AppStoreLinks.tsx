...
export default function AppStoreLinks({ iosUrl, androidUrl, className = '' }: AppStoreLinksProps) {
  ...
  return (
    <div className={`flex flex-col sm:flex-row gap-4 justify-center ${dir === 'rtl' ? 'sm:flex-row-reverse' : ''} ${className}`}>
      {iosUrl && (
        <button
          onClick={() => handleStoreClick(iosUrl)}
          className="flex items-center justify-center bg-gradient-to-r from-gray-900 to-black text-white rounded-xl 
                     px-4 py-3 hover:shadow-lg shadow-md transition-all duration-300 transform 
                     hover:-translate-y-1 hover:scale-[1.02] group"
          aria-label="Download on the App Store"
        >
          <div className="w-8 h-8 mr-2 group-hover:scale-110 transition-transform duration-300">
          ...
          <div className="flex flex-col items-start">
            <span className="text-xs font-light opacity-80 group-hover:opacity-100 transition-opacity">Download on the</span>
      ...
      {androidUrl && (
        <button
          ...
          className="flex items-center justify-center bg-gradient-to-r from-green-600 to-green-700 
                     text-white rounded-xl px-4 py-3 hover:shadow-lg shadow-md transition-all 
                     duration-300 transform hover:-translate-y-1 hover:scale-[1.02] group"
        ...
        >
          ...
          <div className="flex flex-col items-start">
            <span className="text-xs font-light opacity-80 group-hover:opacity-100 transition-opacity">Get it on</span>
